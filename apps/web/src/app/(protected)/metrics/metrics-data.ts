...
export const AD_SET_PERFORMANCE_ROWS: MetricsPerformanceRow[] =
  MOCK_AD_SETS.flatMap((adSet) => {
...
export const CREATIVE_INSIGHT_ROWS: CreativeInsightRow[] = MOCK_ADS.flatMap(
  (ad) => {
...
export const AD_PERFORMANCE_ROWS: MetricsPerformanceRow[] =
  CREATIVE_INSIGHT_ROWS.map((ad) => ({
    id: ad.id,
    level: 'ad',
    name: ad.name,
    status: ad.status,
...
export function getMetricLabel(metricId: CoreMetricId) {
  return MOCK_METRICS.find((metric) => metric.id === metricId)?.title ?? 'ROAS';
}
